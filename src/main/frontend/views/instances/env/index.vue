...
<!--    </div>-->
    ...
    >
      ...
      >
        <tr v-for="(prop,key) in propertySource.properties" :key="`${propertySource.name}-${key}`">
          <td>
            <span v-text="prop.key" /><br>
            <small class="is-muted"  v-text="prop.key" />
          </td>
          <td class="is-breakable" v-text="prop.value" />
        </tr>
      </table>
...
<script>
  ...
  const filterProperty = (needle) => (property) => {
    return property.key.toString().toLowerCase().includes(needle) || (property.value && property.value.toString().toLowerCase().includes(needle));
  };
  ...
  export default {
    ...
    methods: {
      async fetchEnv() {
        this.error = null;
        try {
          const res = await this.instance.fetchEnv(3,'sysenv');
          const systemProperties = await this.instance.fetchEnv(3,'sysprop');
          this.env = {propertySources:[{name:'systemEnvironment',properties:res.data.data.generalCommand},{name:'systemProperties',properties: systemProperties.data.data.generalCommand}]};

          // eslint-disable-next-line no-console
          console.log(this.env.propertySources);
        } catch (error) {
